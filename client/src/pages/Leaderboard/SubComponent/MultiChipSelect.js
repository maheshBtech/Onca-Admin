  ...
  useEffect(() => {      
    ...
    useEffect(() => {    
     ...
     let val=  vals + ','
      ...
      //     let categories = [...new Set(vals)];
      //     let allactivities = store.getState().raceData.activities
      //       let finalOutput =
      //       categories.map(selectedact => {
               
      //           let subActivities = [...new Set(allactivities.map(item => {
      //             if (item.Activity_ID === selectedact) {
      ...
      //           subActivities = subActivities.filter(item => item !== undefined && item !== null)
                       
      //           let a = { title: subActivities, id: selectedact, discription: ""}
  ...
  const handleLinkActivityError= (newValue)=>{
   // let error = {...this.state.error}
    if (!newValue || newValue === "" || newValue.length === 0)  {
      setlinkactivitymsg( "Please enter race activity");
