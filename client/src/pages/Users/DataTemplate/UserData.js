...
const sendPropsToStore = (row)=>{  
  store.dispatch({type:'CHANGE_USER_PROFILE_VALUE', payload:row})
}
const columns = [
  {
    name: 'User Details',
    ...
    wrap: true,
    width: '30%',
    cell: (row) => <div><p className={'m-0'}><Link to="/user-profile" onClick={()=>{sendPropsToStore(row)}}>{row.UserName}</Link></p><p className={'m-0'}>{row.EmaiID}</p><p className={'m-0'}>{row.PhNumber}</p></div>,
  },
  {
    name: 'Location',
    selector: 'Location',
...
modifyDataForTable(){
  if(this.props.userTableData !==""){
    this.state.rawTableData = this.props.userTableData;
...
jsonRaw.forEach((obj, idx)=>{  
  json.User_Name = obj.User_Name
  json.User_SignUp_Date = obj.User_SignUp_Date
  json.user_Active_Flag = obj.user_Active_Flag
  json.Telephone_No = obj.Telephone_No
  json.Pugmarks = obj.Pugmarks
  json.Location_Name = obj.Location_Name
  json.Email_ID = obj.Email_ID
  json.City_Name = obj.City_Name
  json.Activity_Name = obj.Activity_Name
...
this.state.searchedData = modifiedData
}
}
